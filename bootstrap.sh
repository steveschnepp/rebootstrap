...
EOF
	...
	if test "$HOST_ARCH" = arm64ilp32; then
		echo "patching binutils to default to armilp32 and use new triplet. #861833"
		drop_privs patch -p1 <<'EOF'
...
patch_libidn2_0() {
	echo "fix libidn2-0 FTCBFS #857675"
	drop_privs patch -p1 <<'EOF'
--- a/debian/control
+++ b/debian/control
@@ -2,7 +2,7 @@
...
-Build-Depends: debhelper (>= 9), dh-autoreconf, pkg-config, texinfo, texlive, help2man, gengetopt, libunistring-dev
+Build-Depends: debhelper (>= 9), dh-autoreconf, pkg-config, texinfo, texlive, help2man, gengetopt, libunistring-dev, autoconf-archive
 Standards-Version: 3.9.8
 Priority: extra
